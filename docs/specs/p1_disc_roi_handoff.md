...
The chain script is `scripts/train_folds.sh` (idempotent — re-running overwrites logs +
...
nohup zsh scripts/train_folds.sh >/dev/null 2>&1 &
```
...
- Run script: `scripts/train_folds.sh`. Logs: `p1disc_f{0..4}.log`.
- Checkpoints (NOT in git, ~500 MB each): `p1disc_f{0,1}_best.pth` exist; 2-4 pending.
- Design context: `docs/specs/fundus_only_ceiling_design.md`. Analysis scratchpad had `oof.py`,
