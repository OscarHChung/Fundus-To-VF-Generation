...
> ⚠️ **SUPERSEDED (2026-07-15).** This document describes the original **8-sector DRAFT**
> `SECTOR_GRID` + per-sector weights (a non-canonical "GH-style" variant). It is kept for
> provenance — the reported `p1disc` / `p1disc_denoise` checkpoints were trained with this draft's
> mild spatial loss reweighting. The live code now uses the **canonical 6-sector Garway–Heath map
> (Garway-Heath 2000)** with **uniform** sector weights (reporting/figures only); the draft is
> preserved as `LEGACY_SECTOR_GRID_DRAFT`. All 8-sector tables/names below are historical. See
> `docs/specs/2026-07-15-garway-heath-sectoring-fix-design.md`.

Status: ~~draft for review~~ **superseded — historical draft (see banner above)**.
...
- `train()` gained a trailing `deep_cfg=None` param; under
  ...
  resolved values are written into `decoder/garway_heath_sectors.json` under a
