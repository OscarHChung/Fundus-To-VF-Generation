...
- `decoder/tests/test_session3.py` (modify) — add longitudinal correctness tests.
...
- Test: `decoder/tests/test_session3.py`
...
Add to `decoder/tests/test_session3.py`:
...
```bash
rtk git add vf_test_converter.py decoder/tests/test_session3.py
...
Add `test_g1_to_hvf_shape_and_mask()`, `test_followup_visit1_matches_baseline()`, and `test_longitudinal_build_schema()` to the `__main__` call list at the bottom of `decoder/tests/test_session3.py`. Then build:

Run: `rtk proxy python scripts/build_longitudinal_grape.py`
Expected: prints ~631 paired visits / 144 patients / 263 eyes; severe ~104; writes `data/vf_tests/grape_longitudinal.json`.

Run the full suite: `rtk proxy python decoder/tests/test_session3.py`
Expected: `ALL PASSED`.

- [ ] **Step 6: Commit**

```bash
rtk git add build_longitudinal_grape.py decoder/tests/test_session3.py data/vf_tests/grape_longitudinal.json
...
Run: `rtk proxy python decoder/tests/test_session3.py`
Expected: `ALL PASSED`.

- [ ] **Step 5: Commit**

```bash
rtk git add decoder/diagnostics.py decoder/tests/test_session3.py decoder/results/cv_long data/vf_tests/grape_long_train.json data/vf_tests/grape_long_val.json
...
```bash
rtk git add decoder/diagnostics.py decoder/run_cv.py decoder/tests/test_session3.py
...
Log the diagnosis and fix in `docs/history/iterations.md` before re-running the full CV.

- [ ] **Step 5: Record the result and commit logs**

Append a new iteration entry to `docs/history/iterations.md` (hypothesis, config, RAW/CALIB pooled MAE, stratified table, decomposition). Then:

```bash
rtk git add docs/history/iterations.md decoder/results/auto/long_global_cv.json
...
```bash
rtk git add decoder/results/auto/long_disp_cv.json docs/history/iterations.md decoder/results/champion
