...
> `docs/specs/2026-07-11-sub4-fundus-only-vf-design.md`.
...
- Test: `decoder/tests/test_external_papila.py`
...
- [ ] **Step 1: Write the failing test** (`decoder/tests/test_external_papila.py`)
...
- [ ] **Step 2: Run it, verify it fails** — `python -m pytest decoder/tests/test_external_papila.py -q` → FAIL (file missing).
...
- [ ] **Step 5: Run tests, verify pass** — `python -m pytest decoder/tests/test_external_papila.py -q` → PASS.
...
- [ ] **Step 7: Commit** — `git add decoder/build_external_papila.py decoder/tests/test_external_papila.py data/external/papila/SOURCE.md && git commit -m "feat: harmonize PAPILA (fundus+MD) for severity transfer + external validation"`
...
- Test: `decoder/tests/test_denoise_target.py`
...
- [ ] **Step 2: Run it, verify it fails** — `python -m pytest decoder/tests/test_denoise_target.py -q` → FAIL (`hvf_denoised` absent).
...
**Files:** Create `decoder/eval_external_papila.py`. Test: `decoder/tests/test_external_eval.py` (no PAPILA eye is in
