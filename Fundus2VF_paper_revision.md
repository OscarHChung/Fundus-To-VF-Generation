...
`decoder/results/auto/paper_headline_results.md`, `composition_p1disc_denoise.json`, and the OOF
...
> analyzed 24-2 location — attended over the patch tokens under an anatomical prior derived from the
> Garway-Heath optic-disc map, which biases each VF location toward the disc sector that anatomically
> projects to it. A joint global-spatial head added the between-location loss pattern from globally
...
>
> The model was trained to minimize a Garway-Heath sector-weighted regression loss between predicted
> and measured sensitivities, with a per-eye concordance term and value-based reweighting to counter
> the rarity of deeply depressed locations. Two techniques further stabilized training. First,
...
> sensitivity) and per eye, and MAE across the eight Garway-Heath sectors.
>
...
>
> Error was unevenly distributed across the field (Figure 4), lowest centrally and inferiorly and
> higher peripherally and superiorly. By Garway-Heath sector (Table 1), MAE was lowest in the
> inferior-arcuate, inferonasal, and central-inferior sectors (≈3.80 dB) and highest in the
> superior-arcuate and temporal-wedge sectors (≈4.37 dB) — the sectors serving the superior field,
> where glaucomatous defects are common.
>
...
|---|---|
| Inferior-arcuate | 3.80 |
| Inferonasal | 3.80 |
| Central-inferior | 3.80 |
| Nasal-periphery | 4.14 |
| Central-superior | 4.15 |
| Superonasal | 4.25 |
| Superior-arcuate | 4.37 |
| Temporal-wedge | 4.37 |

Legend: Pooled per-location MAE within each Garway-Heath disc sector (out-of-fold, 5-fold CV).
Error is lowest in inferior/central sectors and highest in the superior-arcuate and temporal-wedge
sectors.
...
| Table 1 (sectoral MAE) | Fill | values above |
| Table 2 (comparison) | Fill | values above |
| Table 3 (composition-adjusted) — **NEW** | Add | values above |

Figure legends to update:
- **Figure 2:** "Model pipeline. A disc-centered crop of the fundus photograph is encoded by a frozen
  RETFound vision transformer with trainable low-rank (LoRA) adapters; 52 point-queries attend over
  the patch tokens under a Garway-Heath anatomical prior, with global-spatial and severity-correction
  heads, to predict the 24-2 visual field. Blue = frozen (pretrained); green = trained on GRAPE."
