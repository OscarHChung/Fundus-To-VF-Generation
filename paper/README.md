...
| **2** | `figures/Figure2_architecture.png` | Model pipeline: fundus → disc crop → frozen RETFound (+LoRA) → per-point attention (retinotopic prior + global/severity heads) → 24-2 VF. A dashed "training only" box shows the fundus-only supervision: predicted VF compared to GRAPE ground-truth VF via a canonical 6-sector Garway-Heath sector-weighted loss (GH sectors and measured VF are training-only, never runtime inputs). Blue = frozen, green = trained. | `python decoder/make_architecture_figure.py` → `decoder/results/auto/fig2_architecture.png` |
...
- **Figure 2:** "Model pipeline. A disc-centered crop of the fundus photograph is encoded by a
  ...
  attend over the patch tokens under a fixed retinotopic prior (each query biased toward its
  corresponding peripapillary image region), with global-spatial and severity-correction heads, to
  predict the 24-2 visual field. The model is fundus-only: at inference the sole input is the fundus
  image. During training only (dashed box), the predicted field is compared to GRAPE's measured VF
  (the training label) through a Garway-Heath sector-weighted loss; the canonical six-sector
  Garway-Heath map and the measured VF are used only in training, never as model inputs. Blue =
  frozen (pretrained); green = trained on GRAPE."
...
| # | Files | Content |
|---|---|---|
| **1** | `tables/Table1_sectoral_mae.{md,csv}` | Sectoral MAE across the canonical 6 Garway-Heath sectors (3.79–4.43 dB; lowest temporal/nasal, highest inferotemporal/inferonasal). |
...
- All figures reflect the **frozen leak-free per-patient 5-fold OOF** (`decoder/results/cv_long`)
  ...
  the `p1disc` champion, not re-run for the denoised variant — see `paper/headline_results.md` §4.
